...
function Home() {
  ...
  useEffect(() => {
    const fetchtodos = async () => {
      try {
        setLoading(true);
        const response = await axios.get("https://todo-11-w08p.onrender.com/todo/fetch", {
  ...
  const todoCreate = async () => {
    ...
    try {
      const response = await axios.post(
        "https://todo-11-w08p.onrender.com/todo/create",
        {
          text: newTodo,
          completed: false,
        },
  ...
  const todoStatus = async (id) => {
    ...
    try {
      const response = await axios.put(
        `https://todo-11-w08p.onrender.com/todo/update/${id}`,
        {
          ...todo,
  ...
  const todoDelete = async (id) => {
    try {
      await axios.delete(`https://todo-11-w08p.onrender.com/todo/delete/${id}`, {
        withCredentials: true,
      });
  ...
  const logout = async () => {
    try {
      await axios.get("https://todo-11-w08p.onrender.com/user/logout", {
        withCredentials: true,
      });
